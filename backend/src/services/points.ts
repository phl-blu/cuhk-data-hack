...
export async function getPointsHistory(residentId: string): Promise<PointsTransaction[]> {
  const result = await pool.query<{
    ...
    `SELECT id, points, individual_points, building_points, transaction_type, reference_id, created_at
  ...
  return result.rows.map((row) => ({
    ...
    buildingPoints: row.building_points,
...
export async function upsertResident(
  ...
  // Associate with residential area if not already set
  const raCheck = await pool.query<{ residential_area_id: number | null }>(
    `SELECT residential_area_id FROM residents WHERE id = $1`,
    [residentId]
  );
  if (raCheck.rows[0]?.residential_area_id != null) return;

  // Find nearest housing estate within 500 m of the resident's district centroid
  const nearbyEstate = await pool.query<{ id: number; name: string; district_id: number | null }>(
    `SELECT he.id, he.name, he.district_id
     FROM housing_estates he
     JOIN residents r ON r.id = $1
     JOIN districts d ON d.id = r.district_id
     WHERE ST_DWithin(he.location::geography, d.centroid::geography, 500)
     ORDER BY ST_Distance(he.location::geography, d.centroid::geography)
     LIMIT 1`,
    [residentId]
  );

  let residentialAreaId: number;

  if (nearbyEstate.rows.length > 0) {
    const estate = nearbyEstate.rows[0]!;
    // Find or create a residential_area matching this housing estate
    const raResult = await pool.query<{ id: number }>(
      `INSERT INTO residential_areas (name, district_id, location)
       SELECT he.name, he.district_id, he.location
       FROM housing_estates he WHERE he.id = $1
       ON CONFLICT DO NOTHING
       RETURNING id`,
      [estate.id]
    );
    if (raResult.rows.length > 0) {
      residentialAreaId = raResult.rows[0]!.id;
    } else {
      const existing = await pool.query<{ id: number }>(
        `SELECT ra.id FROM residential_areas ra
         JOIN housing_estates he ON he.name = ra.name AND he.district_id = ra.district_id
         WHERE he.id = $1
         LIMIT 1`,
        [estate.id]
      );
      if (existing.rows.length === 0) return;
      residentialAreaId = existing.rows[0]!.id;
    }
  } else {
    // District-level fallback: find or create a residential_area with name = district name
    if (!districtId) return;
    const fallbackResult = await pool.query<{ id: number }>(
      `INSERT INTO residential_areas (name, district_id)
       VALUES ($1, $2)
       ON CONFLICT DO NOTHING
       RETURNING id`,
    ...
    if (fallbackResult.rows.length > 0) {
      residentialAreaId = fallbackResult.rows[0]!.id;
    } else {
      const existing = await pool.query<{ id: number }>(
        `SELECT id FROM residential_areas WHERE name = $1 AND district_id = $2 LIMIT 1`,
        [districtName, districtId]
      );
      if (existing.rows.length === 0) return;
      residentialAreaId = existing.rows[0]!.id;
  ...
  await pool.query(
    `UPDATE residents SET residential_area_id = $1 WHERE id = $2 AND residential_area_id IS NULL`,
    [residentialAreaId, residentId]
  );
}
